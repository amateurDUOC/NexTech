...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Botón "× Limpiar" por paso
        document.addEventListener('click', function(e) {
            const btn = e.target.closest('.pcgamer-clear-step');
            if (!btn) return;
            e.stopPropagation(); // no disparar toggle del dropdown

            const category  = btn.dataset.category;
            const stepIndex = STEP_ORDER.indexOf(category);

            // Desmarcar selección actual de este paso
            const wrapper = document.querySelector(`.pcgamer-carousel-wrapper[data-category="${category}"]`);
            if (wrapper) {
            ...
            restoreDependentCarousels(category);

            // Re-bloquear todos los pasos posteriores
            if (stepIndex !== -1) {
     ...
     * Si la lista está vacía solo se llega aquí cuando el servidor realmente retornó 0
     * compatibles (no por falta de metadatos, que ahora devuelve todos).
     */
